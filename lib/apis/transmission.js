...
exports.query = function(req,callback){
  ...
  actionSwitch[req.query.action](transmission, req.query.params, callback);
};
...
function cleanFinished(transmission, params, callback){
  transmission.get(function(error,response){
    ...
    response.torrents.forEach(function(item){
      if(item.isFinished || isFinishedButErrored(item)){
        ids.push(item.id);
      }
    });
    transmission.remove(ids, true, function(error, response){
...
function isFinishedButErrored(item){
  return((item.error > 0) && (item.percentDone > 0.99));
}
